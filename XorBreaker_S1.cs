...
public class XorBreaker_S1 {
    ...
    public static KeyValuePair<string, double> Break(byte[] c, int m){
        ...
        for(byte i = 0; i < 128; i++){
            byte[] guess = XorUtil_S1.XOR(c, i);
            double guessScore = Score(guess);
                SCORES.Add(new KeyValuePair<string, double>(HexUtil_S1.ByteArrayToString(guess), guessScore));
            }
    ...
    public static byte BreakChar(byte[] c){
        ...
        for(byte i = 0; i < 128; i++){
            byte[] guess = XorUtil_S1.XOR(c, i);
            double guessScore = Score(guess);
            KeyValuePair<string, double> curr = new KeyValuePair<string, double>(HexUtil_S1.ByteArrayToString(guess), guessScore);
            SCORES.Add(curr);
            l.Add( curr, i);
        }
    ...
    public static KeyValuePair<string, double> Break(byte[] c){
        ...
        for(byte i = 0; i < 128; i++){
            ...
            if (!SCORES.ContainsKey(HexUtil_S1.ByteArrayToString(guess)))
            {
                SCORES.Add(HexUtil_S1.ByteArrayToString(guess), guessScore);
            }
    ...
    public static KeyValuePair<string, double> Break(string[] c){
        ...
        for (int k = 0; k < c.Length; k++){
            KeyValuePair<string, double> guessCurr =  Break(HexUtil_S1.HexStrToByteArray(c[k]),k );
